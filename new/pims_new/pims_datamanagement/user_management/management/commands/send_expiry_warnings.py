...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        users_to_warn = CustomUser.objects.filter(
            ...
            last_password_change__lte=timezone.now()
            - timedelta(days=password_expiry_days - settings.PASSWORD_EXPIRY_WARNING_DAYS),
        )
        ...
        for user in users_to_warn:
            ...
            if days_until_expiry > 0:  # Only send if still some days left before actual expiry
                ...
                email_subject_template = (
                    render_to_string("emails/password_expiry_warning.txt", context)
                    .split("\n")[0]
                    .replace("Subject: ", "")
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Sent password expiry warning to {user.email}. Password expires in {days_until_expiry} days."
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"Skipped sending warning to {user.email} as password has already expired or expires today."
                    )
                )
