...
class AuditLogListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().select_related("user", "user__staff__department", "user__staff__designation")
...
class MyActivityReportView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        breakdown = qs.values("action").annotate(count=Count("action")).order_by("-count")[:10]

        return render(
            request,
            "audit_log/my_activity_report.html",
            {
                "summary": summary,
                "breakdown": breakdown,
...
class ActivityUserSearchView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        html = (
            '<div class="absolute z-50 w-full mt-1 bg-white border border-slate-200 '
...
class FullActivityExportView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        qs = (
            AuditLogEntry.objects.filter(timestamp__gte=since)
            .select_related("user", "user__staff__department", "user__staff__designation", "content_type")
            .order_by("-timestamp")
        )
        ...
        if target_user and (not is_registry or user_id):
            ...
            if s:
                if s.designation:
                    parts.append(s.designation.name.replace(" ", "_"))
                if s.department:
                    parts.append(s.department.name.replace(" ", "_"))
