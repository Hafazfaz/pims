...
class RegistryHubView(RegistryRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if search_query:
            queryset = queryset.filter(Q(title__icontains=search_query) | Q(file_number__icontains=search_query))
        if file_type:
    ...
    def get_context_data(self, **kwargs):
        ...
        if staff_user.is_registry:
            context["pending_access_requests"] = FileAccessRequest.objects.filter(status="pending").order_by(
                "-created_at"
            )
            ...
            for file in outgoing_files_subset:
                ...
                outgoing_list.append({"file": file, "custody_duration": custody_duration, "is_overdue": is_overdue})
                if is_overdue:
                    overdue_count += 1
            ...
            context["docs_added_today"] = AuditLogEntry.objects.filter(
                action="DOCUMENT_ADDED", timestamp__date=today
            ).count()
            context["files_created_today"] = AuditLogEntry.objects.filter(
                action="FILE_CREATED", timestamp__date=today
            ).count()
...
class StaffWithoutFilesView(RegistryRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Staff.objects.exclude(id__in=staff_with_files)
            .exclude(EXCLUDE_REGISTRY_Q)
            .select_related("user", "department", "unit")
...
class CloseMovementView(RegistryRequiredMixin, View):
    ...
    def post(self, request, pk):
        ...
        if not version_ref_id and not file_ref_id:
            messages.error(
                request, "You must reference a previous document version or another file to close this movement."
            )
            return redirect(file_obj.get_absolute_url())
        ...
        log_action(request.user, "MOVEMENT_CLOSED", request=request, obj=file_obj, details={"movement_id": movement.pk})
...
class DispatchDetailView(RegistryRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        movement_history = (
            ...
            .order_by("moved_at")
            if document
            else FileMovement.objects.filter(file=file_obj)
            .select_related("sent_by", "sent_to__user", "sent_to__designation")
            .order_by("moved_at")
        )
        ...
        return render(
            request,
            "document_management/dispatch_detail.html",
            {
                "movement": movement,
                "document": document,
                "file": file_obj,
...
class DocumentsByTypeView(RegistryRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(
            request,
            "document_management/documents_by_type.html",
            {
                "doc_type": dt,
                "documents": documents,
